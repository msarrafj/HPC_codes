...
for i in "${Size[@]}"
	...
	for j in "${name[@]}"
		...
		plot "${file}.dat" using 2:xtic(1) title "SaxpyInnerSimple" linecolor rgb red, \
		    '' using 3 title "SaxpyOuterCritical" linecolor rgb blue, \
		    '' using 4 title "SaxpyOuterSimple" linecolor rgb green
		EOFMarker
	done
done
