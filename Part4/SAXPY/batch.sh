#!/bin/bash
declare -a Code=("SaxpyInnerSimple" "SaxpyOuterCritical" "SaxpyOuterSimple")
...
for i in "${Size[@]}"
	do
	file="Saxpy_$i"
	echo -e "Thread \t SaxpyInnerSimple \t SaxpyOuterCritical \t SaxpyOuterSimple" | tee ./Output/{Time,L1,L2,L3,Power,dram}_${file}.dat 
	echo "size = $i"
	for j in "${Thread[@]}" 
		do
		echo "Threads_used = $j"
		for k in "${Code[@]}"
			...
			rm ./rapl-read ./allMat
			cat ./resultsrun.dat >> ./resultsMaster.dat
