...
for i in "${Size[@]}"
	...
	for j in "${name[@]}"
		...
		plot "${file}.dat" using 2:xtic(1) title "blockSize=4" linecolor rgb red, \
		    '' using 3 title "BlockSize=64" linecolor rgb blue, \
		    '' using 4 title "BlockSize=1024" linecolor rgb green
		EOFMarker
	done
done
