...
for i in "${Size[@]}"
	...
	for j in "${name[@]}"
		...
		set title "${j} for 3 matrix sizes using MKL"
		plot "${file}.dat" using 2:xtic(1) title "matrixSize=1024" linecolor rgb red, \
		    '' using 3 title "matrixSize=4096" linecolor rgb blue, \
		    '' using 4 title "matrixSize=16384" linecolor rgb dark_green
		EOFMarker
	done
done
